...
MotionUpsampling::resample( Int iMbXCurr, Int iMbYCurr )
{
  ...
  RNOK( xSetPartIdcArray() );//根据对应的基本层的4x4block的intra和inter类型，设置partidc（基本层的inter类型的4x4block的宏块索引8x8索引4x4索引的组合）intra类型的先置为-1，然后根据周边的inter的替代。

  if( m_bInCropWindow && ! m_bIntraBL )
  {
    for( Int iListIdx = 0; iListIdx < m_iMaxListIdx; iListIdx++ )
    {
      RNOK( xGetRefIdxAndInitialMvPred( ListIdx( iListIdx ) ) );//获取基本层的refidx,mv并对mv上采样，如果不是1:2，则用4个4x4子块的最小的refidx作为增强层的8x8的refidx
    }
    for( Int iB8x8Idx = 0; iB8x8Idx < 4; iB8x8Idx++ )
    {
      RNOK( xDeriveBlockModeAndUpdateMv( iB8x8Idx ) );//若不为1:2且进行子宏块划分则根据4x4块的mv进行分割判断并对分割后的mv进行后处理
    }
    RNOK( xDeriveMbMode       () );//根据refidx和mv进行分割处理
    RNOK( xDeriveFwdBwd       () );//b帧p帧
...
MotionUpsampling::xSetPartIdcArray()
{
  ...
  //===== determine all 16 initial partition indexes =====
  {
    ...
    {
      RNOK( xGetRefLayerPartIdc( ( iX << 2 ) + 1, ( iY << 2 ) + 1, m_aaiPartIdc[iX][iY] ) );//按（1,1）点对应的基本层4x4子块来确定
      m_bIntraBL  = ( m_bIntraBL  && ( m_aaiPartIdc[iX][iY] == -1 ) );  //只要有一个子块是inter，则 m_bIntraBL 就不成立
    }
    if( m_bIntraBL ) //子块对应的都是intra块
    {
      m_eMbMode  = INTRA_BL;
    }
  }
  ...
  //针对4x4块和8x8块出现参考是I块的情况，用邻近4x4块的参考层对应块取代，这样就可以防止计算参考索引上采样出现 -1 的情况。
  //===== replace values of "-1" on a 4x4 block basis =====
  {
    ...
    for( Int iXP = 0; iXP < 2; iXP++ )
    {
      ...
      for( Int iXS = 0; iXS < 2; iXS++ )
      {
        ...
        if( m_aaiPartIdc[iXC][iYC] == -1 )//针对4x4块出现参考是I块的情况
        {
          Int iYSInv  = 1 - iYS;
          Int iXSInv  = 1 - iXS;
          ...
          if( ! aabProcI4x4Blk[iXSInv][iYS] && m_aaiPartIdc[iXCInv][iYC] != -1 ) //采用垂直临近块的PartIdc值
          {
            m_aaiPartIdc[iXC][iYC] = m_aaiPartIdc[iXCInv][iYC];
          }
          else if( ! aabProcI4x4Blk[iXS][iYSInv] && m_aaiPartIdc[iXC][iYCInv] != -1 )//采用水平临近块的PartIdc值
          {
            m_aaiPartIdc[iXC][iYC] = m_aaiPartIdc[iXC][iYCInv];
          }
          else if( ! aabProcI4x4Blk[iXSInv][iYSInv] && m_aaiPartIdc[iXCInv][iYCInv] != -1 )//采用对角块的PartIdc值
          {
  ...
  //===== replace values of "-1" on an 8x8 block basis =====
  {
    ...
    for( Int iXP = 0; iXP < 2; iXP++ )
    {
      ...
      if( m_aaiPartIdc[iXO][iYO] == -1 ) //8x8块的idc==-1 说明 8x8块的4个4X4块的IDC都是-1
      {
        aabProcI8x8Blk[iXP][iYP] = true;

        if( ! aabProcI8x8Blk[iXPInv][iYP] && m_aaiPartIdc[iXOInv][iYO] != -1 )  // 对8x8块的4个4X4块的IDC分别处理
        {
...
MotionUpsampling::xGetRefIdxAndInitialMvPred( ListIdx eListIdx )
{
  ...
  for( Int i8x8BlkX = 0; i8x8BlkX < 2; i8x8BlkX++ )
  {
    ...
    for( Int i4x4BlkY = 0; i4x4BlkY < 2; i4x4BlkY++ )//按照当前层8x8块为最小处理单元获取运动矢量上采样。获取每个4x4块的运动矢量和参考索引之后，
    for( Int i4x4BlkX = 0; i4x4BlkX < 2; i4x4BlkX++ )//从4个参考索引中取最小非负数为该8x8块的参考索引
    {
    ...
    for( Int iYS = 0; iYS < 2; iYS++ )//
    for( Int iXS = 0; iXS < 2; iXS++ )//8x8块的refidx 由4个4x4块中的最小的refidx得来
    {                                 //找到refidx最小的4x4块，将此块的mv作为8x8块的mv
...
MotionUpsampling::xDeriveBlockModeAndUpdateMv( Int i8x8BlkIdx )
{
  ...
  //===== derive partition size =====
  {
    for( Int iListIdx = 0; iListIdx < m_iMaxListIdx; iListIdx++ )
    {
      Bool  bHor1Match  = ( xMvDiff( m_aaacMv[iListIdx][iXO  ][iYO  ], m_aaacMv[iListIdx][iXO+1][iYO  ] ) <= iAbsMvDiffThreshold );//增强层的子块的分割，不考虑refidx是否相等，只考虑mv的情况
      ...
      b8x8Match         = b8x8Match && bHor1Match && bVer1Match && bDiagMatch;//如果4个运动矢量的差的绝对值小于等于1，则对4个运动矢量取均值，得到最终的运动矢量。
      bHorMatch         = bHorMatch && bHor1Match && bHor2Match;//如果在4x4块排列的水平方向上，两组2个4x4块的运动矢量差的绝对值小于等于1，则分别对2个4x4块的运动矢量取均值，8x8块的分割模式为8x4。
      bVerMatch         = bVerMatch && bVer1Match && bVer2Match;//如果在4x4块排列的垂直方向，两组2个4x4块的运动矢量差的绝对值小于等于1，则分别对2个4x4块的运动矢量取均值，8x8块的分割模式为4x8。
    }
    const BlkMode aiBlkMode[4]  = { BLK_4x4, BLK_8x4, BLK_4x8, BLK_8x8 }; //子宏块分割模式
    m_aeBlkMode[i8x8BlkIdx]     = aiBlkMode[ b8x8Match ? 3 : bHorMatch ? 1 : bVerMatch ? 2 : 0 ];
  }
  ROTRS( m_cMvScale.m_bRSChangeFlag,          Err::m_nOK );//在空间分辨率变化为1：2的情况下，参考层的1个4x4块对应当前层的一个8x8块，因此运动矢量的后处理不存在。
                                                           //只有在限制空间分辨率为0的情况下（比如空间分辨率为1：1.5），当前层的每个8x8块覆盖参考层的多个4x4块，才存在运动矢量后处理。
  ROTRS( m_aeBlkMode[i8x8BlkIdx] == BLK_4x4,  Err::m_nOK );

  //===== combine motion vectors =====
  {
    for( Int iListIdx = 0; iListIdx < m_iMaxListIdx; iListIdx++ ) //运动矢量后处理
    {
      switch( m_aeBlkMode[i8x8BlkIdx] )
      {
	  case BLK_8x8: //对4个运动矢量取均值，得到最终的运动矢量
        {
      ...
      case BLK_8x4: //分别对2个4x4块的运动矢量取均值
        {
      ...
      case BLK_4x8: //分别对2个4x4块的运动矢量取均值
        {
...
MotionUpsampling::x8x8BlocksHaveSameMotion( ListIdx eListIdx, Int i8x8IdxA, Int i8x8IdxB )
{
  ...
  ROFRS( m_aaaiRefIdx[eListIdx][i8x8IdxA&1][i8x8IdxA>>1] == m_aaaiRefIdx[eListIdx][i8x8IdxB&1][i8x8IdxB>>1], false );//保证refidx相同
...
MotionUpsampling::xDeriveMbMode()
{
  ...
  Bool  bHorMatch   = true;
  Bool  bVerMatch   = true;
  for( Int iListIdx = 0; iListIdx < m_iMaxListIdx; iListIdx++ )//判断宏块分割模式
  {
    bHorMatch = bHorMatch && x8x8BlocksHaveSameMotion( ListIdx( iListIdx ), 0, 1 );//先判断refidx是否相等，在判断mv是否相等。都相等则返回true
...
MotionUpsampling::xDeriveFwdBwd()
{
  ...
  {
    m_uiFwdBwd <<= 4;
    m_uiFwdBwd  += ( m_aaaiRefIdx[0][n&1][n>>1] > 0 ? 1 : 0 );// 11 01 10 00
  ...
  for( Int iX = 0; iX < 4 && m_bResPredSafe; iX++ )
  {
    ...
    for( Int iListIdx = 0; iListIdx < 2 && m_bResPredSafe; iListIdx++ )
    {
      ...
      if( m_bResPredSafe )
      {
        ...
        for( k = 0; k < 4 && m_bResPredSafe; k++ )  // 根据mv的差值判断是否残差上采样
        {
