  ...
  //========== horizontal upsampling ===========
  {
    for( int j = 0; j < iBaseH; j++ )
    {
      ...
      for( int i = 0; i < iCurrW; i++ )
      {
        if( i < iLOffset || i >= iCurrW - iROffset )
        {
          m_paiTmp1dBuffer[i] = 0; // set to zero 临时保存
          continue;
        }
        ...
        int iTBlkIdc0   = m_paiTransBlkIdc[ iRefPosY * m_iImageStride + iRefPosX0 ];//判断是否跨块？
        int iTBlkIdc1   = m_paiTransBlkIdc[ iRefPosY * m_iImageStride + iRefPosX1 ];

        if( iTBlkIdc0 == iTBlkIdc1 )  // iRefPosX0 == iRefPosX1  同一块
        {
