                                   ...
                                   UInt               uiSliceId /* = MSYS_UINT_MAX */ )
{
  ...
  for( UInt uiMbX = 0; uiMbX < uiFrmWidth;  uiMbX++ )
  {
    ...
    if( ! bMbAffFrame )
    {
      RNOK( pcMbDataCtrl->getBoundaryMask( uiMbY, uiMbX, bIntra, uiMask, uiSliceId ) );//获得周边宏块的inter/intra类型
      if  ( ! bIntra )//当前宏块为inter 当为不受限的时候强制为intra
      {
        YuvMbBufferExtension  cBuffer;//暂时保存inter宏块及周边的一行预测像素
        YuvPicBuffer*         pcPicBuffer = pcFrame->getPic( ePicType )->getFullPelYuvBuffer(); //pcFrame中的像素Buffer
        cBuffer.setAllSamplesToZero ();//处理前先清零
        if( uiMask )
        {
          cBuffer.loadSurrounding   ( pcPicBuffer );//从pcPicBuffer中加载当前inter宏块的四周的一行像素到cBuffer的luma中
          RNOK( xPadRecMb           ( &cBuffer, uiMask ) );//在cBuffer中利用周边的intra块对当前inter块进行赋值
        }
        pcPicBuffer->loadBuffer     ( &cBuffer ); //从cBuffer中加载处理完毕的inter宏块的数据到pcPicBuffer的相应位置
      }
    }
    else
    {
