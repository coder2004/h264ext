                   ...
                   Bool           bReconstructAll )
{
  ...
  if( rcMbDataAccess.isTCoeffPred() && rcMbDataAccess.getMbData().getMbMode() == INTRA_BL )//继承了基本层的分割模式预测方向，解码按照基本的intra类型解码
  {//isTCoeffPred需要连续一致？
    ...
    {//当第一层为Intra4x4时，第二层为intra_bl，第三层满足此条件时，mbmode继承第二层还是intra_bl，但是此时的intraPredMode是什么呢？还是tcoeff_level_prediction_flag从第二层开始都是1但可以以后不为1
